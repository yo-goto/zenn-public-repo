...
通常 `Promise.all()` には Promise インスタンスの配列を引数として渡しますが、このようにイテラブルオブジェクトであればなんでも渡せます。内部的にイテラブルオブジェクトの要素の数だけマイクロタスクが発生し、それら全ての結果を待って履行状態となるので、非同期的に解決されることになります。(`Promise.all()` の引数に何も渡さなかった場合は、同期的に解決される)
詳細が知りたい方は、ECMAScriptの仕様をご確認下さい。
