...
aliases: [ch_then メソッドのコールバックで非同期処理]
---

# このチャプターについて
このチャプターは、別のチャプター『then メソッドのコールバックで Promise インスタンスを返す』の続きとしての内容となります。
...
:::message
副作用の概念は React の関数コンポーネントなどででてくるので覚えておくと良いでしょう。
:::
...
Promise インスタンスから解決値を取り出す方法としては、実はもう１つ await 式がありますが、詳細はここでは解説しません。『Promise チェーンから非同期関数へ』のチャプターで詳しく解説します。
