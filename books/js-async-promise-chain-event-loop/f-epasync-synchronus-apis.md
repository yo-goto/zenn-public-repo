...
- 非同期 API(Non-blocking API) : 書いた順番通りに実行されないため処理の流れがわかりづらく制御が難しいが、同時に複数のことができるため効率が良い。関連する作業の順序を制御するための書き方が、Callback のネスト形式や Promise chain、async/await となる。
:::
