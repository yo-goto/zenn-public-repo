...
:::message
上記の記事はイベントループの仕様に基づいているのでかなり正確な情報だと思います。私の本よりよほど情報信頼性があるので、この本で情報の足りない部分があったり困ったことがあったらこちらの記事を参考にするのがよいと思います。ただし、仕様に近いことも有り(ほぼ仕様)、ある程度のメンタルモデルができていないと最初は難しいかもしれません。
:::
...
:::message alert
こちらの内容についてはこの本の最後で再度まとめていますので、今は理解できなくても大丈夫です。
:::

非同期処理の本質的な部分を一言で表現すると「イベントループにおけるタスクをマイクロタスクの処理」です。

「A したら B する、B したら C する」というような逐次処理をそれぞれ非同期的に実行したいなら、タスクやマイクロタスクを連鎖(chain)させます。

コールバックベースの API による Callback hell についてはいわば「**タスク連鎖(Task chain)**」といえるでしょう。イベントループ上でタスクの連鎖的処理を行うことで非同期の逐次処理を行うことができます。

Promise チェーンや async/await、Promise-based API などについては Promise インスタンスを連鎖させることで非同期的に逐次処理を行えます。いわば「**マイクロタスク連鎖(Microtask chain)**」といえるでしょう。イベントループ上でマイクロタスクの連鎖的処理を行うことで非同期の逐次処理を行うことができます。

"タスク連鎖"や"マイクロタスク連鎖"は私が作成した造語ですが(本質的な部分を捉えるためにつくったただの言葉です)、これが理解できるとイベントループのメンタルモデルが盤石になると思います。
