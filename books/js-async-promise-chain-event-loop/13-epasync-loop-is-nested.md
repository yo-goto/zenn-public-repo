...
:::message
今後のチャプターでは、`v8` コマンドを使用して V8 エンジンでそのまま JavaScript を実行する場合がでてきます。
:::
...
:::message
ヒントとして、`"[L] 👦 MICRO: then"` の部分は引っ掛けです。グローバルコンテキストがわかっていればマイクロタスクの実行タイミングが分かるはずです。

わからない場合には、『コールスタックと実行コンテキスト』のチャプターを確認してください。
:::
