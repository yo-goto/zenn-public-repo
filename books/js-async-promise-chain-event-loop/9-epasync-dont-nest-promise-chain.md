...
ここで考えるのは親のコールバック関数 `cb1` を登録していた `then(cb1)` から返される Promise インスタンスです。この `cb1` から返される Promise インスタンスが解決されないままだと `then(cb1)` から返される Promise インスタンスの状態が履行状態にはならず待機状態のままで、次の `then(cb2)` メソッドのコールバック関数 `cb2` をマイクロタスクキューへと送ることができません。
