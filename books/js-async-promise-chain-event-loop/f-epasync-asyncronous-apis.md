...
例えば、次のコードでは、タイマーを順番に起動しますが。タイマーの処理が終わっていないにも関わらず次のタイマーを起動できたり、コンソールに出力したりできるのは何故でしょうか？
...
タイマーによって「時間を測る」ということは明らかに作業です。時間を図る作業がメインスレッドで行われていたら**メインスレッドはブロッキングされてその間は何もできません**。もしそうならタイマーの処理などほぼ意味がありませんね。

```js
setTimeout(() => {
  console.log("1000ミリ秒測るタイマー");
}, 1000);
// もしブロッキングされていたら次の処理はタイマーが終わってからでないと実行できない
consle.log("実際には時間を図りつつ他のことができる"); 
```

現実では、時間を図りつつ他の JavaScript コードを処理しているわけで、**同時に複数のことをやっている**わけです。

というわけで、時間を測るという作業やインターネットを介したデータ取得などの行為は実は API を介して**環境が代わりにバックグラウンドで並列的(parallel)に行ってくれます**。つまり、 API の呼び出しは**環境へ作業を委任する(delegate)という行為**だった訳です。
...
:::message
「並列」とは言い切れないパターンがあったりしますが、**JavaScript (JS エンジン)の視点で考えれば別のところで同時に起きていること**を理解するには「並列」として便宜的に考えないと理解が進まないというジレンマがあります。厳密な定義の並列を使って一般的に説明しようとするとおそらく使えなくなるので、お茶を濁すしかないというのが実情な気がしています。
:::
...
非同期 API は非同期処理かどうかというと、**むしろ並列処理**です。非同期 API の起動自体はソースコードの配置に沿ってタイミングをずらさずに(同期的に)開始しますが、時間的にはバックグラウンドで処理を(並列的に)継続しつつ、メインスレッドでは別の JS コードの処理ができています。そして、非同期 API の結果から生じたデータに対して後で何かするというのが非同期処理です。ソースコードの配置に沿った順番とはずれたタイミングで(非同期に)処理されます。

:::message
なぜ「非同期 API」という呼称なのか?

非同期 API の処理が並列的にバックグラウンドで行われることやそれを使ってやりたいことそのものが「複数の同時実行」なので、「非同期 API(Asynchronous API)」ではなく「並列 API(Parallel API)」という呼称の方が直感的には分かりやすいと思うのですが、なぜ「非同期 API」と呼ばれているのか正直よくわかりません。非同期 API の完了自体は非同期的に終わるのでこれを指して非同期と言っているのか、API で生じたデータに対して非同期処理があとで付随するから非同期 API と呼んでいるという可能性もあります。

以下のように非同期 API が起動してから完了するまでの経過を見ると「同期的」も「並列的」も「非同期的」もあると言えてしまいます。

- 1. 非同期 API の起動 : 同期的
- 2. 非同期 API の処理 : 並列的(これがメインでメリット)
- 3. 非同期 API の完了 : 非同期的
- 4. 非同期 API の結果を使って次に行う処理: 非同期的かつ並行的

そもそも API 自体が環境の提供する機能なので並列的が当たり前であり、ブロッキングする「同期 API(Synchronous API)」が特殊で、その API に対抗する否定概念としてブロッキングしない「非・同期 API(A・synchronous API)」であるということなのかもしれません。それなら「Not-synchronous」ぐらいの方が分かりやすいですが。Web API とかもそうですが、名前に信頼がおけない用語が多いです。

正直言って「非同期、同期、並列、並行」などの概念で考えるとなんとでも言えてこねくり回せるので、学習開始時や混乱する場合はすべて「**ブロッキングするかしないか**」だけで考えた方がよいです。「非同期 API / 同期 API」という呼称は理解の妨げになる可能性があり信頼できないので、代わりに「Non-blocking API / Blocking API」でメインに考えた方が分かりやすいかもしれません。
:::
...
以下のコードでは、`fetch()` でデータ取得しながら `setTimeout()` で指定した時間 1000 ミリ秒を計測し、コンソールにもテキスト出力しています。**環境の機能である非同期 API(Non-blocking API)を使用しているから同時に複数のことができます**。「同時に複数のことをやることで効率的な処理がしたい」という欲求を叶えるために非同期 API を使用し、後続の関連する処理を制御するために非同期処理のシンタックスである Callback や Promise chain, async/await を使用します。
...
注意点として、`fetch()` や `setTimeout()` は「非同期関数」と呼称されることがあります。これらは環境から提供される関数であり非同期 API であるため「非同期関数」と呼ぶことは確かに間違いではないですが、誤解を招きやすい表現となっています。例えば、`async` キーワードを使って宣言された async function のことも「非同期関数」と呼ぶので非常に紛らわしいことになります。`setTimeout()` と async function を同じ呼び方で同一視することは絶対にできないので分けて考えるべきです。

この本では「非同期関数」といったら `async` キーワードを使って宣言・定義された関数のことを指し、ECMAScript の範疇ではなく環境の機能ならば「API」として呼んでいるので注意してください。
