...
以下のように非同期 API が起動してから完了するまでの経過を見ると「同期」も「並列」も「非同期」もあると言えてしまいます。

- 1. 非同期 API の起動 : 同期的
- 2. 非同期 API の処理 : 並列的(これがメインでメリット)
- 3. 非同期 API の完了 : 非同期的

そもそも API 自体が環境の提供する機能なので並列が当たり前であり、ブロッキングする「同期 API(Synchronous API)」が特殊で、その API に対抗する否定概念としてブロッキングしない「非・同期 API(A・synchronous API)」であるということなのかもしれません。
:::
