...
執筆当初はプロミスチェーン(Promise chain)とイベントループ(Event loop)という大きく２つのポイントを軸に非同期処理を解説する本と制作していましたが、現在では以下のように非同期処理のメカニズム全体を理解するための横断的なストーリーを解説するような本となっています。

![必要な知識のライン](/images/js-async/img_async-story-line.png)

ただし、学習経験から重要な情報に沿ってロードマップ的な内容を再構成した作りとなっているので内容重視の解説順番となります。図にあるように、非同期処理を理解するために必要であることが明らかではない知識というものが実はありますが、まずはここから解説し始めるような本となっています。
...
この本は大きく分けて４つの章から構成されています。上の「非同期処理のメカニズムを理解するために必要な知識」の図にある内容について学習における重要な順番で解説しています。
