...
:::message
ちなみに asynchronous の発音記号は `/eɪsɪ́ŋkr(ə)nəs/` なので、`async` の読み方は日本語だと"エイシンク"になります。
:::
...
Promise chain で書き直した方は async 関数内部で Promise 処理を `return` していないので副作用が生じていますし、気づきにくいですが発生するマイクタスクの数が異なっています。Promise chain と async/await を発生するマイクタスクの数を同じにした上で同じ実行順序に書き直すのは相当に内部を理解していないと難しいです。
:::
