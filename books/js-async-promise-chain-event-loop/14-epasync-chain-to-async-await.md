...
Promise chain について詳しくなった所でようやく async/await に入ることができます。このチャプターでは、async/await を学習する上での注意点や意識すべき点などについて解説します。
...
上コードのコメントはイメージしやすいように書いたものです。次のチャプター『[V8 エンジンによる async/await の内部変換](15-epasync-v8-converting)』でも解説しますが、実際には次のように async 関数の実行時に await 式において現在実行しているコンテキストが中断して、await 評価が終了した後に、分割された実行コンテキストが処理再開を告げるマイクロタスクとして処理されます。

![実行フローの分割](/images/js-async/img_asyncfunc-splited.jpg)

`then()` を使ってもう少し分かりやすく以下のようなコードで考えてみましょう。実行フローが分割されるとは、結局のところは各 await 式において `then()` の処理が起きているということです。
...
環境が代行する非同期 API の並列的作業の完了を「待つ」ことについて先に言及してしまいましたが、そもそも await 式の「待つ」の意味は **実際にそこですべての処理を停止させているわけではない** のでかなり注意する必要があります。
...
現段階でいきなり理解するのは難しいかもしれませんが、そのようなケースで制御がどのように移動しているかを追ってみると次のようになります。`console.log()` 内の数字を追うことで制御の流れがなんとなく分かると思います。実際に実行すると数字の順番にコンソールへ出力が行われます (コードそのものに特殊な意味はありません)。
...
この場合 async function から返ってくる Promise インスタンス自体が拒否状態となります。従って、チェーンしている `then()` メソッドのコールバックは実行されずに、`catch()` メソッドによって例外として捕捉されてコールバックが実行されます。
...
これは async 関数内で例外を throw した場合と同じです。それ以降の処理はスキップされて、async 関数から返ってくる Promise インスタンスは拒否状態となり、`catch()` メソッドで拒否理由と共に例外が捕捉されます。
...
async 関数では古典的な例外捕捉の方法として try/catch/finally を使用できます。
...
このようにした場合、try/catch で例外捕捉するため、async 関数から返ってくる Promise インスタンスは履行状態となります。したがって、チェーンしている `then()` メソッドのコールバックは実行でき、逆に `catch()` メソッドのコールバックは実行されません。
...
### await 式は Promise インスタンスでないものも評価できる
