...
図から `Strict` も判別可能なユニオン型であると言えます。実際 `Strict` 型は `Sum` 型の subtype であり、どちらの型も交差(共通要素)がないことがわかります。図から `Sum` 型を抜いて表示すると `Strict` 型は次のように Disjoint union であり交差を持ちません。
...
というのも、先程述べたとおり Narrowing はユニオン型という和集合から合成元の型(集合)を取り除いていく行為でもあります。すべての構成要素となる型を取り除くと空集合、つまり何も要素が無い集合となるので最終的には `never` 型となります。構成要素をすべて列挙しないと `never` 型にすることはできないので、この性質を利用してあえてエラーを起こして網羅しているかのチェックをするのが網羅性チェック(Exausitveness check)です。
