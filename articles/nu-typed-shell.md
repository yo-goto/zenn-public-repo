...
![エラーメッセージの画像](/images/nushell/img_nushell-error.png) *Nushellでの分かりやすいエラーメッセージ*

使い始めてからまだ1ヶ月ぐらいですが、かなり奥が深く一つの記事で解説しきるのは難しいので、この記事では基本体な設定と型とコマンドについて重点をおいて最後は具体的なカスタムコマンドの定義をいくつか取り上げて解説したいとおもいます。

## Nushellとは

Nushell とは "**A new type of shell**" を謳っている通り、伝統的なUnixシェルとは異なるモダンなシェルです。読み方はそのまま new shell[^0] (新しいシェル)で、[Rust](https://www.google.com/url?sa=t&rct=j&q=&esrc=s&source=web&cd=&ved=2ahUKEwid5Lqfit6IAxV4s1YBHV_VMrwQFnoECAkQAQ&url=https%3A%2F%2Fwww.rust-lang.org%2Fja&usg=AOvVaw0PogFTF2sEQdAfjFAaO8L4&opi=89978449) 言語で開発されており、以下のような特徴があります。

[^0]: 公式で読み方が記載されていませんが、[Youtubeの動画](https://youtu.be/pNPQ4_tTKuc?si=HMg8_XsGMyIkit_b)で聞く限り new と発音していますね
...
開発者には、[TypeScript の開発](https://devblogs.microsoft.com/typescript/announcing-typescript-1-0/)や、[Rust のエラーメッセージのデザイン](https://blog.rust-lang.org/2016/08/10/Shape-of-errors-to-come.html)などに携わっていた Sophia J. Turner という方がコアメンバーとして入っており(現在は管理メンバーから外れて引き継いだとのこと[^1])、Nushell は TypeScript と Rust の息吹が随所に感じられる言語となっています。Sophia 氏の次のブログ記事では Nushell が bash/zsh/fish/PowerShell とどのように違うか、POSIX にしない理由、構造化データがなぜ重要なのかといった理由が語られています。

[^1]: https://www.sophiajt.com/following-new-paths-ahead/

https://www.sophiajt.com/case-for-nushell/

Sophia 氏のインタビュー動画

https://youtu.be/qpIHMr-A4yU?si=oDyyNNdfyqIwS1F7
...
:::details fishの設定
fish shell(とfisher)を使っている人は気付いたかもしれませんが、これはfishでの設定と同じ構成になるようにしています。
...
#          ^^^^^^ パラメータの型はintとして型注釈
> foo "bar" # string型の値をパラメータとして渡すと型エラーとなる
...
さらに LSP や IDE サポートを提供しているため、シェルスクリプトにも関わらずコマンドの入出力についての型推論や変数への型注釈、注釈省略時のインレイヒント、定義ジャンプ、ホバー表示などの機能を使うことができます。
...
Nushellでは以下のような基本型が提供されており、この他にも特殊な型が複数校用意されています。
...
ただし、パイプラインの入出力は関数オーバーロードのように複数個のパターンを取ることができるので、そのような場合にはシグネチャは以下のようになります。

```nu
def command-name [
  param: ParamType
]: [InputType1 -> OutputType1, InputType2 -> OutputType2, ...] {
  # ...
}
```
